...
public:
	...
	UnitTest() : VulkanRenderer()
	{
		...
		animtime.endTimer = 0.00001f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 0.041f;
		animtime.endTimer = 0.620f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 0.670f;
		animtime.endTimer = 3.164f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 3.250f;
		animtime.endTimer = 4.790f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 4.8770f;
		animtime.endTimer = 7.0f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 7.048f;
		animtime.endTimer = 9.160f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 9.250f;
		animtime.endTimer = 10.5810f;
		animationTimer.push_back(animtime);
		//
		animtime.startTimer = 10.6310f;
		animtime.endTimer = 13.3690f;
		animationTimer.push_back(animtime);
	}
	...
	void LoadScene()
	{
		...
		AllModel allModel;
		//Load Model 
		allModel.modelData.LoadModel("HumanRun.fbx", vertexLayoutAnimation, 1.00f, vulkanDevice, queue.graphicQueue);
		allModel.materialData.albedoTexture.LoadFromFile("ArissaAlbedo.dds", VK_FORMAT_B8G8R8A8_UNORM, vulkanDevice, vulkanHelper, queue.graphicQueue);
		allModel.materialData.normalTexture.LoadFromFile("ArissaNormal.dds", VK_FORMAT_B8G8R8A8_UNORM, vulkanDevice, vulkanHelper, queue.graphicQueue);
		allModel.materialData.specularTexture.LoadFromFile("ArissaSpecular.dds", VK_FORMAT_B8G8R8A8_UNORM, vulkanDevice, vulkanHelper, queue.graphicQueue);
	...
	void UpdateAnimationBuffer()
	{
			...
			//humanModelAnimation.model->animation->Update(timer);
			...
			for (uint32_t i = 0; i < transforms.size(); i++)
			{
				...
				//bones.push_back(((humanModelAnimation.model->animation->aiToGlm(transforms[i]))));
			}
			//for (uint32_t i = 0; i < humanModelAnimation.model->animation->boneTransforms.size(); i++)
			//{
			//	//bones.push_back(glm::transpose((glm::make_mat4(&(transforms[i].a1)))));
			//	bones.push_back(glm::transpose((glm::make_mat4(&(transforms[i].a1)))));
			//}
	...
	void PreparePipelines()
	{
		...
		std::string vertAddress = fileName + "src\\AnimationKeyFrame(Quaternions)\\Shaders\\GBuffer.vert.spv";
		std::string fragAddress = fileName + "src\\AnimationKeyFrame(Quaternions)\\Shaders\\GBuffer.frag.spv";
	...
	// won't see anything rendered to the attachment
		...
		vertAddress = fileName + "src\\AnimationKeyFrame(Quaternions)\\Shaders\\Deferred.vert.spv";
		fragAddress = fileName + "src\\AnimationKeyFrame(Quaternions)\\Shaders\\Deferred.frag.spv";
	...
	virtual void GuiUpdate(GUI * gui)
	{
		...
		if (gui->Header("Animation Data"))
		{
			...
			if (gui->SliderInt("Select Animation Count", &animationCount,0,7))
			{

			}
			if (gui->InputFloat("Animation Timer", &timer, 0.001f, 3))
			{
